...
class CachePerformanceTester {
    ...
    constructor() {
        ...
        // Create repositories
        this.usersRepo = new UsersRepository(this.prisma, {
            organizationId: 'test-org-id',
            userId: 'test-user-id'
        }, this.cache);
        this.orgSettingsRepo = new OrganizationSettingsRepository(this.prisma, {
            organizationId: 'test-org-id',
            userId: 'test-user-id'
        }, this.cache);
    }
    ...
    async testUserByIdCaching() {
        ...
        const withoutCacheDurations = [];
        for (let i = 0; i < iterations; i++) {
            const start = Date.now();
            // Create a new repository without cache
            const noCacheRepo = new UsersRepository(this.prisma, {
                organizationId: 'test-org-id',
                userId: 'test-user-id'
            });
        ...
        const result = this.calculateTestResult('User by ID Caching', withCacheDurations, withoutCacheDurations);
        this.testResults.push(result);
    }
    /**
     * Test organization settings caching performance
     */
    async testOrgSettingsCaching() {
        ...
        for (let i = 0; i < iterations; i++) {
            const start = Date.now();
            const noCacheRepo = new OrganizationSettingsRepository(this.prisma, {
                organizationId: 'test-org-id',
                userId: 'test-user-id'
            });
        ...
        const result = this.calculateTestResult('Organization Settings Caching', withCacheDurations, withoutCacheDurations);
        this.testResults.push(result);
    }
    /**
     ...
     */
    calculateTestResult(testName, withCacheDurations, withoutCacheDurations) {
...
//# sourceMappingURL=cache_performance_test.js.map