...
interface PrismaViolation {
  ...
  line: number;
  content: string;
  type: 'import' | 'usage' | 'reference';
}

class PrismaForbiddenChecker {
  private violations: PrismaViolation[] = [];
  private readonly rootDir = process.cwd();

  /**
   * Check for Prisma violations in the codebase
   */
  async checkForViolations(): Promise<void> {
   ...
   */
  private async checkDocumentationFiles(): Promise<void> {
    const docPatterns = [
      'docs/**/*.md',
      '*.md',
      'plans/**/*.md',
   ...
   */
  private async checkPackageFile(filePath: string): Promise<void> {
    try {
      const content = readFileSync(filePath, 'utf8');
      const lines = content.split('\n');
   ...
   */
  private async checkDocumentationFile(filePath: string): Promise<void> {
    try {
      const content = readFileSync(filePath, 'utf8');
      const lines = content.split('\n');
