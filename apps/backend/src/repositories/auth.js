...
export class AuthRepository {
    ...
    async findUserByEmail(tenantId, email) {
        try {
            const result = await this.db
                ...
                .from(users)
                .where(and(eq(users.email, email.toLowerCase()), eq(users.status, 'active'), eq(users.organizationId, tenantId) // Tenant-scoped lookup
            ))
                .limit(1);
     ...
     */
    async getUserAuthContext(userId, tenantId) {
        try {
            // Get user basic info
            const userResult = await this.db
                ...
                .from(users)
                .where(and(eq(users.id, userId), eq(users.status, 'active'), eq(users.organizationId, tenantId) // Tenant-scoped
            ))
                .limit(1);
            if (!userResult[0]) {
                return null;
     ...
     */
    async hasUserMembership(userId, tenantId) {
        try {
            const result = await this.db
