...
function extractSessionId(request: FastifyRequest): string | null {
  ...
  const sessionCookie = (request as any).cookies?.['pf-session'];
  if (sessionCookie) {
...
async function authPlugin(fastify: FastifyInstance) {
   ...
   */
  async function verifyOpaqueToken(token: string): Promise<AuthContext | null> {
    try {
   ...
   */
  async function verifyPasetoToken(token: string): Promise<AuthContext | null> {
    try {
  ...
  fastify.decorate('getUserId', (request: FastifyRequest): string | null => {
    return request.auth?.userId || null;
  });

  // Authentication middleware for routes
  fastify.decorate('authenticate', async (request: FastifyRequest, reply: FastifyReply) => {
    logger.info({ url: request.url, hasCache: !!(fastify as any).cache }, 'Authenticate middleware called');
