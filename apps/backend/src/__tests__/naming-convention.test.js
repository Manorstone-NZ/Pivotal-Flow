...
import { organizations, users, customers, projects, quotes, quoteLineItems, rateCards, serviceCategories, roles, permissions, userRoles, orgSecurityPolicies, orgNotificationPrefs, orgFeatureFlags, orgSettings, auditLogs, currencies } from '../lib/schema.js';
describe('Database Schema Naming Convention', () => {
    describe('snake_case convention validation', () => {
        ...
        const checkTableColumns = (tableName, table) => {
    ...
    describe('Specific naming convention rules', () => {
        it('should use snake_case for all timestamp fields', () => {
            ...
            allTables.forEach(table => {
                const columnNames = Object.values(table).map((col) => col?.name).filter(Boolean);
                timestampPatterns.forEach(pattern => {
                    const matchingColumns = columnNames.filter((col) => col.includes(pattern));
                    matchingColumns.forEach(col => {
