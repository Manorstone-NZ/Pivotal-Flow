...
function getRoutePermission(method, url) {
    ...
    const exactPermission = ROUTE_PERMISSIONS[exactKey];
    if (exactPermission) {
 ...
 */
export async function accessControlMiddleware(request) {
 ...
 */
async function verifyJwtToken(_token) {
 ...
 */
export async function tenancyMiddleware(request) {
    const user = request.user;
    if (!user?.org) {
        return; // Skip for public routes
    }
    // Add organization context to request
    request.organizationId = user.org;
...
export function requestContextMiddleware(request, reply, done) {
    ...
    if (request.user?.org) {
        reply.header('X-Organization-ID', request.user.org);
    }
    done();
}
