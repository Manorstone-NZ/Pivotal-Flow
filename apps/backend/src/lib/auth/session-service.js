...
export class SessionService {
     ...
     */
    async createSession(userId, tenantId, _sessionData, options = {}) {
    ...
    async validateSession(sessionId) {
        try {
            ...
            const sessionResult = await this.db
                ...
                .where(and(eq(sessions.id, sessionId), isNull(sessions.revokedAt), // Not revoked
    ...
    async revokeSession(sessionId, reason) {
        try {
            const result = await this.db
                ...
                .where(and(eq(sessions.id, sessionId), isNull(sessions.revokedAt) // Only revoke if not already revoked
            ));
    ...
    async revokeUserSessions(userId, reason) {
        try {
            const result = await this.db
                ...
                .where(and(eq(sessions.userId, userId), isNull(sessions.revokedAt) // Only revoke if not already revoked
            ));
    ...
    async getUserSessions(userId) {
        try {
            const result = await this.db
                ...
                .from(sessions)
                .where(and(eq(sessions.userId, userId), gt(sessions.expiresAt, new Date()) // Not expired
            ))
