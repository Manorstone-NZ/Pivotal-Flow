...
export class ConfigError extends Error {
    missingKeys;
    invalidValues;
    constructor(message, missingKeys = [], invalidValues = {}) {
        super(message);
...
const RedisConfigSchema = z.object({
    REDIS_URL: z.string().refine((val) => val.startsWith('redis://') || val.startsWith('rediss://'), { message: 'REDIS_URL must start with redis:// or rediss://' }),
});
const DatabaseConfigSchema = z.object({
    DATABASE_URL: z.string().refine((val) => val.startsWith('postgresql://'), { message: 'DATABASE_URL must start with postgresql://' }),
});
...
export function loadConfig() {
    ...
    if (nodeEnv !== 'development') {
        ...
        if (missingKeys.length > 0) {
            throw new ConfigError(`Missing required environment variables in ${nodeEnv} environment`, missingKeys);
        }
    }
    // Validate CORS_ORIGIN for non-development environments
    if (nodeEnv !== 'development' && env['CORS_ORIGIN'] === '*') {
        throw new ConfigError('CORS_ORIGIN cannot be wildcard (*) in non-development environments');
    }
    try {
        const rawConfig = {
            server: {
        ...
        const xeroEnabled = !!(validatedConfig.xero.XERO_CLIENT_ID &&
        ...
        if (!fileTokenSecret) {
            if (nodeEnv === 'development') {
                ...
                throw new ConfigError('FILE_TOKEN_SECRET is required in non-development environments', ['FILE_TOKEN_SECRET']);
            }
        }
        const config = {
            ...validatedConfig,
        ...
        if (error instanceof z.ZodError) {
            ...
            throw new ConfigError('Invalid environment variable values', [], invalidValues);
        }
        throw error;
    }
}
