...
export class ProjectService {
  ...
  async listProjects(filters?: ProjectFilters): Promise<ProjectsListResponse> {
    try {
      ...
      if (filters?.search) {
        whereConditions.push(
          sql`(${projects.name} ILIKE ${`%${filters.search}%`} OR ${projects.code} ILIKE ${`%${filters.search}%`})`
        );
      }
      ...
      const data: Project[] = projectsList.map(project => ({
        id: project.id,
  ...
  async getProjectById(id: string): Promise<ProjectDetailResponse | null> {
    ...
    const serviceCategoriesList: any[] = [];
