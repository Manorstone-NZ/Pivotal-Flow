 ...
 */
function getRateLimitKey(userContext, endpoint) {
 ...
 */
function checkRateLimit(userContext, endpoint) {
    const now = Date.now();
 ...
 */
export async function portalRateLimit(request, reply) {
    // Extract user context from authenticated request
    const userContext = request.portalUser;
    if (!userContext) {
 ...
 */
export async function portalAuth(request, reply) {
 ...
 */
export function getRateLimitStatus(userContext, endpoint) {
    const key = getRateLimitKey(userContext, endpoint);
