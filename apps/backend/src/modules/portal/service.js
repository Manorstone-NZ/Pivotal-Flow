...
import { quotes, quoteLineItems, invoices, invoiceLineItems, users } from '../../lib/schema.js';
import { PermissionService } from '../permissions/service.js';
import { PORTAL_PERMISSIONS, PORTAL_CONFIG } from './constants.js';
/**
 * Portal Service
 *
...
export class PortalService extends BaseRepository {
    ...
    async validatePortalAccess() {
        ...
        const user = await this.db
            ...
            .from(users)
            .where(and(eq(users.id, this.userContext.userId), eq(users.organizationId, this.userContext.organizationId), eq(users.customerId, this.userContext.customerId), eq(users.userType, 'external_customer'), isNull(users.deletedAt)))
            .limit(1);
        if (user.length === 0) {
            await this.logSecurityViolation({
    ...
    async getQuotes(filters) {
        ...
        const portalQuotes = quotesData.map(quote => ({
            id: quote.id,
    ...
    async getQuoteDetail(quoteId) {
        ...
        const quoteResult = await this.db
            ...
            .from(quotes)
            .where(and(eq(quotes.id, quoteId), eq(quotes.organizationId, this.userContext.organizationId), eq(quotes.customerId, this.userContext.customerId), isNull(quotes.deletedAt)))
            .limit(1);
        ...
        const portalLineItems = lineItemsResult.map(item => ({
            id: item.id,
    ...
    async getInvoices(filters) {
        ...
        const invoicesData = await this.db
            ...
            .offset(offset);
        // Transform to portal format
        const portalInvoices = invoicesData.map(invoice => ({
            id: invoice.id,
    ...
    async getInvoiceDetail(invoiceId) {
        ...
        const invoiceResult = await this.db
            ...
            .from(invoices)
            .where(and(eq(invoices.id, invoiceId), eq(invoices.organizationId, this.userContext.organizationId), eq(invoices.customerId, this.userContext.customerId), isNull(invoices.deletedAt)))
            .limit(1);
