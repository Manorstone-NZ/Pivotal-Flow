...
export class InvoiceService {
    ...
    async generateInvoiceNumber() {
        ...
        const latestInvoice = await this.db
            ...
            .from(invoices)
            .where(and(eq(invoices.organizationId, this.context.organizationId), like(invoices.invoiceNumber, `${prefix}-%`)))
    ...
    async listInvoices(filters = {}) {
        ...
        if (filters.search) {
            whereConditions.push(sql `(${invoices.invoiceNumber} ILIKE ${`%${filters.search}%`} OR ${invoices.title} ILIKE ${`%${filters.search}%`})`);
        }
        // Get total count
    ...
    async getInvoiceById(id) {
        ...
        const invoiceResult = await this.db
            ...
            .from(invoices)
            .leftJoin(customers, eq(invoices.customerId, customers.id))
            .where(and(eq(invoices.id, id), eq(invoices.organizationId, this.context.organizationId)))
            .limit(1);
     ...
     */
    async updateInvoiceStatus(id, transition) {
        const existingInvoice = await this.getInvoiceById(id);
        if (!existingInvoice) {
            return null;
        }
