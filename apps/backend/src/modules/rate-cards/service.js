...
import { validateRateCardData, validateRateCardItemData } from './schemas.js';
 ...
 */
function buildCacheKey(organizationId, resource, action, identifier) {
 ...
 */
function validateMetadataJSONB(data, context) {
...
export class RateCardService extends BaseRepository {
    options;
    auditLogger;
    constructor(db, options, auditLogger) {
        super(db, options);
     ...
     */
    async getActiveRateCard(date = new Date()) {
        const dateStr = date.toISOString().split('T')[0];
        // Build cache key as per requirements
        const cacheKey = buildCacheKey(this.options.organizationId, 'ratecard', 'active');
        ...
        const result = await this.db
            .select()
            .from(rateCards)
            .where(and(eq(rateCards.organizationId, this.options.organizationId), eq(rateCards.isActive, true), sql `${rateCards.effectiveFrom} <= ${dateStr}`, or(isNull(rateCards.effectiveUntil), sql `${rateCards.effectiveUntil} >= ${dateStr}`)))
    ...
    async getRateCardItems(rateCardId) {
        // Build cache key as per requirements
        const cacheKey = buildCacheKey(this.options.organizationId, 'rateitem', undefined, rateCardId);
        // Try to get from cache first
        try {
            const cachedResult = await this.getFromCache(cacheKey);
            if (cachedResult) {
        ...
        const result = await this.db
            .select()
            .from(rateCardItems)
            .where(and(eq(rateCardItems.rateCardId, rateCardId), eq(rateCardItems.isActive, true)));
        // Cache the result
        try {
     ...
     * TTL: 300 seconds as per requirements
     */
    async getRateCardItemByCode(itemCode) {
        // Build cache key as per requirements
        const cacheKey = buildCacheKey(this.options.organizationId, 'rateitem', 'code', itemCode);
        // Try to get from cache first
        try {
            const cachedResult = await this.getFromCache(cacheKey);
            if (cachedResult) {
        ...
        const result = await this.db
            ...
            .where(and(eq(rateCardItems.itemCode, itemCode), eq(rateCardItems.isActive, true), eq(rateCards.organizationId, this.options.organizationId), eq(rateCards.isActive, true)))
            .orderBy(desc(rateCards.isDefault), desc(rateCards.effectiveFrom))
            .limit(1);
     ...
     */
    async resolvePricing(lineItems, userHasOverridePermission = false, effectiveDate = new Date()) {
        const results = [];
        const errors = [];
            ...
            try {
                ...
                // Priority 2: Match by itemCode (SKU) then description
                    ...
                    if (rateItem) {
                        ...
                        const rateItem = this.findMatchingRateItemByDescription(rateCardItemsList, item.description);
                        if (rateItem) {
                            result = {
                                unitPrice: new Decimal(rateItem.baseRate),
                ...
                // Priority 3: Match by service category and description
                    ...
                    const rateItem = this.findMatchingRateItem(rateCardItemsList, item.serviceCategoryId, item.description);
                    if (rateItem) {
                        result = {
                            unitPrice: new Decimal(rateItem.baseRate),
                            taxRate: new Decimal(rateItem.taxClass === 'exempt' ? 0 : 0.15),
     ...
     */
    findMatchingRateItem(rateItems, serviceCategoryId, _description) {
        // First try exact service category match
        const match = rateItems.find(item => item.serviceCategoryId === serviceCategoryId &&
            item.isActive);
        if (match) {
            return match;
        }
     ...
     */
    findMatchingRateItemByDescription(rateItems, _description) {
        // Find the best match by description similarity
        // This is a basic implementation - could be enhanced with fuzzy matching
        return rateItems.find(item => item.isActive) || null;
    }
    /**
     ...
     */
    async updateRateCard(rateCardId, data) {
     ...
     */
    async updateRateCardItem(rateCardItemId, data) {
     ...
     * Get rate card by ID
     */
    async getRateCardById(rateCardId) {
        const result = await this.db
            .select()
            .from(rateCards)
            .where(and(eq(rateCards.id, rateCardId), eq(rateCards.organizationId, this.options.organizationId)))
            .limit(1);
     ...
     */
    async getRateCardByIdWithTx(tx, rateCardId) {
     ...
     */
    async getRateCardItemById(rateCardItemId) {
        const result = await this.db
            .select()
            .from(rateCardItems)
            .where(eq(rateCardItems.id, rateCardItemId))
     ...
     */
    async getRateCardItemByIdWithTx(tx, rateCardItemId) {
        const result = await tx
            .select()
            .from(rateCardItems)
            .where(eq(rateCardItems.id, rateCardItemId))
     ...
     */
    async listRateCards(options = { page: 1, pageSize: 20 }) {
        const { page = 1, pageSize = 20, search } = options;
        const offset = (page - 1) * pageSize;
        let whereClause = eq(rateCards.organizationId, this.options.organizationId);
        if (search) {
            ...
            if (rateCards.description) {
                ...
                const searchClause = or(nameMatch, descriptionMatch);
                whereClause = and(whereClause, searchClause);
            }
            else {
                whereClause = and(whereClause, nameMatch);
            }
        }
     ...
     */
    async bustRateCardCache(rateCardId) {
        try {
            // Bust active rate card cache
            const activeRateCardKey = buildCacheKey(this.options.organizationId, 'ratecard', 'active');
            await this.deleteFromCache(activeRateCardKey);
            // Bust rate card items cache
            const rateCardItemsKey = buildCacheKey(this.options.organizationId, 'rateitem', undefined, rateCardId);
