...
export class F2RateCardService {
  ...
  constructor(
    private context: F2RateCardContext,
    private auditLogger?: AuditLogger
  ) {}

  async getAllRateCards(query: RateCardListQuery = {}): Promise<RateCardListResponse> {
    ...
    if (search) {
      whereConditions.push(
        or(
          like(f2RateCards.name, `%${search}%`),
          like(f2RateCards.description, `%${search}%`)
        )!
      );
    }
    ...
    const rateCardsWithServices = await Promise.all(
      rateCards.map(async (rateCard) => {
        const services = await this.db
          ...
          .from(f2Services)
          .where(and(
            eq(f2Services.rateCardId, rateCard.id),
            eq(f2Services.isActive, true)
          ))
  ...
  async getRateCard(id: string): Promise<RateCardResponse | null> {
    const rateCard = await this.db
      .select()
      .from(f2RateCards)
      .where(and(
        eq(f2RateCards.id, id),
        eq(f2RateCards.organizationId, this.context.organizationId),
        eq(f2RateCards.tenantId, this.context.tenantId)
      ))
      .limit(1);
  ...
  async createRateCard(data: CreateRateCard): Promise<RateCardResponse> {
    const id = generateId();
    const now = new Date();
  ...
  async updateRateCard(id: string, data: UpdateRateCard): Promise<RateCardResponse | null> {
  ...
  async createService(rateCardId: string, data: CreateService): Promise<ServiceResponse> {
    const id = generateId();
    const now = new Date();
  ...
  async updateService(rateCardId: string, serviceId: string, data: UpdateService): Promise<ServiceResponse | null> {
    ...
    await this.db
      ...
      .set(updateData)
      .where(and(
        eq(f2Services.id, serviceId),
        eq(f2Services.rateCardId, rateCardId)
      ));
  ...
  async deleteService(rateCardId: string, serviceId: string): Promise<boolean> {
    const existing = await this.getService(rateCardId, serviceId);
    if (!existing) {
      return false;
    }
  ...
  async getService(rateCardId: string, serviceId: string): Promise<ServiceResponse | null> {
  ...
  // Helper methods
  private formatRateCardResponse(rateCard: any, services: any[]): RateCardResponse {
    return {
      id: rateCard.id,
