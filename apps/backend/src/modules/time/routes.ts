...
interface SubmitTimeEntryRequest {
  Params: { id: string };
}

interface ApproveTimeEntryRequest {
  Params: { id: string };
  Body: {
    comments?: string;
  };
...
export const timeRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  // List time entries
  fastify.get<ListTimeEntriesRequest>('/v1/time-entries', {
    schema: {
      querystring: {
        type: 'object',
        properties: {
  ...
  // Create time entry
  fastify.post<CreateTimeEntryRequest>('/v1/time-entries', {
    schema: {
      body: {
        type: 'object',
  ...
  fastify.post<SubmitTimeEntryRequest>('/v1/time-entries/:id/submit', {
    schema: {
      params: {
        type: 'object',
        required: ['id'],
    ...
    try {
      ...
      const entry = await db
        ...
        .where(and(
          ...
          user.roles.includes('admin') ? undefined : eq(timeEntries.userId, user.userId)
        ))
        .limit(1);
  ...
  // Approve time entry
  fastify.post<ApproveTimeEntryRequest>('/v1/time-entries/:id/approve', {
    schema: {
      params: {
        type: 'object',
        required: ['id'],
  ...
  // Reject time entry
  fastify.post<RejectTimeEntryRequest>('/v1/time-entries/:id/reject', {
    schema: {
      params: {
        type: 'object',
        required: ['id'],
