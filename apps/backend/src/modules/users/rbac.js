 ...
 */
export async function canViewUsers(user, _fastify) {
    try {
 ...
 */
export async function canManageUsers(user, _fastify) {
    try {
        // Admin users have all permissions
        if (user.roles.includes('admin')) {
            return { hasPermission: true };
 ...
 */
export async function canAccessUser(user, targetUserId, fastify) {
    try {
        // Check if target user exists in same organization
        const targetUserResult = await fastify.db
            .select()
            .from(users)
            .where(and(eq(users.id, targetUserId), eq(users.organizationId, user.organizationId), isNull(users.deletedAt)))
            .limit(1);
 ...
 */
export async function canModifyUser(user, targetUserId, fastify) {
    try {
        // Check if target user exists in same organization
        const targetUserResult = await fastify.db
            .select()
