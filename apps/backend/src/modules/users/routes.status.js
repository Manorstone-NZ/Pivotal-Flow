...
import { userStatusSchema } from './schemas.js';
import { getUserById, updateUser } from './service.drizzle.js';
export const updateUserStatusRoute = async (fastify) => {
        ...
        try {
            ...
            if (!isActive && currentUser.roles.some((role) => role.name === 'admin' && role.isActive)) {
                const adminCountResult = await fastify.db
                    ...
                    .where(and(ne(userRoles.userId, targetUserId), eq(roles.name, 'admin'), eq(roles.isActive, true), eq(users.organizationId, userContext.organizationId), eq(users.status, 'active'), isNull(users.deletedAt)));
