...
export class ApprovalService extends BaseRepository {
    permissionService;
    auditLogger;
    constructor(db, options, fastify) {
        super(db, options);
    ...
    async createApprovalRequest(data) {
        ...
        const hasPermission = await this.permissionService.hasPermission(actorId, 'approvals.request');
        ...
        const approverHasPermission = await this.permissionService.hasPermission(data.approverId, 'approvals.decide');
        ...
        const existingRequest = await this.db
            ...
            .where(and(eq(approvalRequests.entityType, data.entityType), eq(approvalRequests.entityId, data.entityId), eq(approvalRequests.status, APPROVAL_STATUS.PENDING)))
            .limit(1);
    ...
    async approveRequest(requestId, data) {
        ...
        const actorId = required(this.options.userId, "authenticated user id missing");
        const hasPermission = await this.permissionService.hasPermission(actorId, 'approvals.decide');
        if (!hasPermission.hasPermission) {
        ...
        const [request] = await this.db
            .select()
            .from(approvalRequests)
            .where(and(eq(approvalRequests.id, requestId), eq(approvalRequests.organizationId, this.options.organizationId)));
        if (!request) {
     ...
     */
    async cancelRequest(requestId, data) {
        // Get the approval request
        const [request] = await this.db
            .select()
            .from(approvalRequests)
    ...
    async getApprovalRequests(filters = {}) {
        ...
        const actorId = required(this.options.userId, "authenticated user id missing");
        const hasPermission = await this.permissionService.hasPermission(actorId, 'approvals.view');
        if (!hasPermission.hasPermission) {
        ...
        for (const setting of settings) {
            switch (setting.key) {
                case APPROVAL_POLICY_KEYS.QUOTE_SEND_REQUIRES_APPROVAL:
                    policy.quoteSendRequiresApproval = setting.value;
                    break;
                case APPROVAL_POLICY_KEYS.INVOICE_ISSUE_REQUIRES_APPROVAL:
                    policy.invoiceIssueRequiresApproval = setting.value;
                    break;
                case APPROVAL_POLICY_KEYS.PROJECT_CLOSE_REQUIRES_APPROVAL:
                    policy.projectCloseRequiresApproval = setting.value;
                    break;
            }
        }
        return policy;
     ...
     */
    async requiresApproval(entityType, action) {
     ...
     */
    async updateEntityStatus(entityType, entityId, decision) {
