 ...
 */
export const pasetoServiceRoutes: FastifyPluginAsync = async (fastify) => {
      ...
      try {
        ...
        switch (purpose) {
          ...
          case 'quote-delivery':
            ...
            token = await pasetoService.generateQuoteDeliveryToken(
              request.body.resource, tenantId, serviceId
            );
          ...
          case 'invoice-access':
            ...
            token = await pasetoService.generateInvoiceAccessToken(
              request.body.resource, tenantId, serviceId
            );
  ...
  fastify.post<{
    Body: { token: string; consumeToken?: boolean };
    Reply: any;
  }>(
    "/verify-token",
    {
  ...
  fastify.post<{
    Body: { quoteId: string; tenantId: string; userId: string };
    Reply: any;
  }>(
    "/quote-delivery-token",
    {
      schema: {
        body: Type.Object({
          quoteId: Type.String(),
