...
export const pasetoLoginRoute = async (fastify) => {
    fastify.post("/login", {
        ...
        logger.info({
            hasFastify: !!fastify,
            hasTokenService: !!(fastify.tokenService),
        ...
        const tokenService = fastify.tokenService || new (await import('../../lib/tokens/paseto-service.js')).PasetoTokenService(fastify);
        ...
        try {
            ...
            try {
                const testResult = await fastify.db.execute('SELECT 1 as test');
...
//# sourceMappingURL=routes.login.paseto.js.map