 ...
 */
export function registerQuoteLineItemRoutes(fastify) {
    ...
    // Delete line item
    fastify.delete('/v1/quotes/:quoteId/line-items/:lineItemId', async (request, reply) => {
        try {
            // Get user context
            const user = request.user;
            if (!user) {
    ...
    fastify.get('/v1/quotes/:quoteId/pdf', async (request, reply) => {
        try {
            // Get user context
            const user = request.user;
            if (!user) {
...
// Submit quote route
export function registerSubmitQuoteRoute(fastify) {
    fastify.post('/v1/quotes/:quoteId/submit', async (request, reply) => {
        try {
            // Get user context
            const user = request.user;
            if (!user) {
...
//# sourceMappingURL=routes.line-items.js.map