...
export class QuoteNumberGenerator {
     ...
     */
    async generateQuoteNumber(organizationId) {
        return withTx(this.db, async (tx) => {
            ...
            const existingQuotes = await tx
                ...
                .from(quotes)
                .where(and(eq(quotes.organizationId, organizationId), isNull(quotes.deletedAt)));
            let nextSequence = 1;
     ...
     */
    async getOrganizationPrefix(_organizationId) {
     ...
     */
    static validateQuoteNumber(quoteNumber) {
        // Format: PREFIX-YEAR-SEQUENCE (e.g., Q-2024-0001)
     ...
     */
    static parseQuoteNumber(quoteNumber) {
     ...
     */
    async quoteNumberExists(quoteNumber, organizationId) {
        const result = await this.db
            .select({ id: quotes.id })
            .from(quotes)
            .where(and(eq(quotes.quoteNumber, quoteNumber), eq(quotes.organizationId, organizationId), isNull(quotes.deletedAt)))
            .limit(1);
     ...
     */
    async getQuoteNumberStats(organizationId) {
        const currentYear = new Date().getFullYear();
        const prefix = await this.getOrganizationPrefix(organizationId);
        // Get total quotes
        ...
        const yearResult = await this.db
            .select({ count: quotes.id })
            .from(quotes)
            .where(and(eq(quotes.organizationId, organizationId), eq(quotes.quoteNumber, `${prefix}-${currentYear}-%`), isNull(quotes.deletedAt)));
