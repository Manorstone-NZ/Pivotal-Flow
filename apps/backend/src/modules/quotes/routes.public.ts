...
interface AcceptQuoteRequest {
  ...
  Body: Static<typeof AcceptQuoteSchema>;
}
...
interface RejectQuoteRequest {
  ...
  Body: Static<typeof RejectQuoteSchema>;
}
...
async function validateTokenAndGetQuote(fastify: FastifyInstance, token: string) {
  ...
  const isValidTenant = QuoteDeliveryService.getOrganizationFromTokenHash(
    tokenValidation.organizationHash!,
    organization.id
  );
 ...
 */
export function registerPublicQuoteRoutes(fastify: FastifyInstance): void {
  ...
  }, async (request: FastifyRequest<PublicQuoteRequest>, reply: FastifyReply) => {
    try {
  ...
  }, async (request: FastifyRequest<AcceptQuoteRequest>, reply: FastifyReply) => {
    try {
      const { token } = request.params;
  ...
  }, async (request: FastifyRequest<RejectQuoteRequest>, reply: FastifyReply) => {
    try {
      const { token } = request.params;
