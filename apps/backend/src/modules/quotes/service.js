...
import { QuoteStatus, isValidStatusTransition, validateQuoteData } from './schemas.js';
...
export class QuoteService extends BaseRepository {
    ...
    async createQuote(data) {
        ...
        try {
            ...
            return withTx(this.db, async (tx) => {
                ...
                const pricingResolution = await this.rateCardService.resolvePricing(data.lineItems.map(item => {
                    const mappedItem = {
                    ...
                    return mappedItem;
                }), canOverridePrice.hasPermission, new Date(data.validFrom));
     ...
     */
    async transitionStatus(quoteId, transition) {
     ...
     */
    async getQuoteByIdWithTx(tx, quoteId) {
        const result = await tx
            .select()
            .from(quotes)
            .where(and(eq(quotes.id, quoteId), eq(quotes.organizationId, this.options.organizationId), isNull(quotes.deletedAt)))
            .limit(1);
        if (result.length === 0) {
            return null;
        }
    ...
    async listQuotes(pagination, filters = {}) {
        ...
        try {
            ...
            const quotesWithLineItems = await Promise.all(quotesResult.map(async (quote) => {
                const lineItems = await this.db
                    .select()
                    .from(quoteLineItems)
     ...
     */
    async calculateQuoteDebug(input) {
        // Validate quote data
        ...
        const pricingResolution = await this.rateCardService.resolvePricing(input.lineItems.map(item => ({
            lineNumber: item.lineNumber,
            description: item.description,
            ...(item.unitPrice && {
                unitPrice: {
        ...
        })), permissionCheck.hasPermission, new Date(input.validFrom));
        if (!pricingResolution.success || !pricingResolution.results) {
            const errorDetails = pricingResolution.errors?.map(e => `Line ${e.lineNumber}: ${e.reason}`).join('; ');
            throw new Error(`Pricing resolution failed: ${errorDetails}`);
        }
