...
function createQuoteData(overrides = {}) {
    const baseData = {
...
describe('Quote Integration Tests', () => {
    ...
    describe('Quote Workflow Tests', () => {
        it('should create quote with proper status and calculations', async () => {
            ...
            const webDevItem = lineItems.find((item) => item.description === 'Web Development');
    ...
    describe('Audit Logging Tests', () => {
        it('should log all quote operations', async () => {
            ...
            const auditLogsResult = await testDb.select().from(auditLogs).where(and(eq(auditLogs.entityType, 'Quote'), eq(auditLogs.entityId, quote.id), eq(auditLogs.organizationId, 'test-org-1')));
            ...
            const updateLogs = await testDb.select().from(auditLogs).where(and(eq(auditLogs.entityType, 'Quote'), eq(auditLogs.entityId, quote.id), eq(auditLogs.action, 'quotes.update')));
            expect(updateLogs).toHaveLength(1);
            // Status transition
            await quoteService.transitionStatus(quote.id, { status: QuoteStatus.PENDING });
            // Check for status transition audit log
            const statusLogs = await testDb.select().from(auditLogs).where(and(eq(auditLogs.entityType, 'Quote'), eq(auditLogs.entityId, quote.id), eq(auditLogs.action, 'quotes.status_transition')));
