...
export class AdminTenantService {
    ...
    async listTenants(query) {
        ...
        const { page = 1, limit = 20, search, status = 'ALL', sortBy = 'createdAt', sortOrder = 'desc' } = query;
        const offset = (page - 1) * limit;
        // Build where conditions
        const whereConditions = [];
        if (search) {
            whereConditions.push(sql `(${tenants.name} ILIKE ${`%${search}%`} OR ${tenants.slug} ILIKE ${`%${search}%`} OR ${tenants.billingEmail} ILIKE ${`%${search}%`})`);
        }
        if (status !== 'ALL') {
        ...
        const tenantsWithCounts = await this.db
            ...
            membershipCount: sql `COUNT(${memberships.id})`
        })
            .from(tenants)
            ...
            .limit(limit)
            .offset(offset);
        const formattedTenants = tenantsWithCounts.map((tenant) => ({
            id: tenant.id,
            name: tenant.name,
            slug: tenant.slug,
    ...
    async getTenantDetails(tenantId) {
        ...
        const formattedMemberships = tenantMemberships.map((membership) => ({
            id: membership.id,
        ...
        const [revenueStats] = await this.db
            .select({
            totalRevenue: sql `COALESCE(SUM(${invoices.totalAmount}), 0)::text`
        })
            .from(invoices)
    ...
    async createTenant(data) {
        ...
        const [newTenant] = await this.db.transaction(async (tx) => {
     ...
     */
    async addMembership(tenantId, data) {
        await this.verifyPlatformAdminAccess();
        ...
        const [newMembership] = await this.db.transaction(async (tx) => {
     ...
     */
    async verifyPlatformAdminAccess() {
...
export const createAdminTenantService = (options) => {
