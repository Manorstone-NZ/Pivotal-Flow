...
describe('Reports Integration Tests', () => {
    ...
    describe('Quote Cycle Time Report with Seed Data', () => {
        it('should generate report with proper envelope shape and totals', async () => {
            ...
            const totalPercentage = result.cycleTimeDistribution.reduce((sum, item) => sum + item.percentage, 0);
    ...
    describe('Performance and Observability', () => {
        ...
        it('should record Prometheus metrics', async () => {
            const metrics = reportingService.metrics;
            ...
            await reportingService.generateQuoteCycleTimeSummary(filters);
            expect(recordReportGeneratedSpy).toHaveBeenCalledWith(REPORT_TYPES.QUOTE_CYCLE_TIME, 'test-org-id');
            expect(recordReportDurationSpy).toHaveBeenCalledWith(REPORT_TYPES.QUOTE_CYCLE_TIME, 'test-org-id', expect.any(Number));
        });
    });
