...
import { EXPORT_JOB_STATUS, REPORT_TYPES } from './constants.js';
...
export class ExportJobService {
    ...
    async createExportJob(request) {
        // Check permissions
        const canExport = await this.permissionService.hasPermission(this.userId, 'reports.export_reports');
    ...
    async getExportJobStatus(jobId) {
        const job = await this.db
            .select()
            .from(exportJobs)
            .where(and(eq(exportJobs.id, jobId), eq(exportJobs.organizationId, this.organizationId)))
            .limit(1);
        if (job.length === 0) {
     ...
     */
    async listExportJobs(page = 1, limit = 25) {
        const offset = (page - 1) * limit;
        const [jobs, totalResult] = await Promise.all([
            this.db
                .select()
                .from(exportJobs)
                .where(and(eq(exportJobs.organizationId, this.organizationId), eq(exportJobs.userId, this.userId)))
    ...
    async cancelExportJob(jobId) {
        const result = await this.db
            ...
            .where(and(eq(exportJobs.id, jobId), eq(exportJobs.organizationId, this.organizationId), eq(exportJobs.userId, this.userId), eq(exportJobs.status, EXPORT_JOB_STATUS.PENDING)));
        if (result.length === 0) {
    ...
    async processExportJob(jobId) {
        ...
        try {
            ...
            // Record metrics
            this.metrics.recordExportCompleted(exportJob.reportType, exportJob.format, this.organizationId, durationMs);
            // Log audit event
            await this.auditLogger.logEvent({
                organizationId: this.organizationId,
                userId: this.userId,
            ...
            const durationMs = Date.now() - startTime;
            // Record metrics
            this.metrics.recordExportFailed('unknown', 'unknown', this.organizationId, 'processing_error');
            // Log audit event
            await this.auditLogger.logEvent({
                organizationId: this.organizationId,
                userId: this.userId,
     ...
     */
    async generateQuoteCycleTimeData(_filters) {
     ...
     */
    async generateInvoiceSettlementTimeData(_filters) {
        // Placeholder implementation
        return [
            {
                invoiceId: 'i1',
     ...
     */
    async generateTimeApprovalsData(_filters) {
        // Placeholder implementation
     ...
     */
    async generatePaymentsReceivedData(_filters) {
        // Placeholder implementation
        return [
            {
                paymentId: 'p1',
