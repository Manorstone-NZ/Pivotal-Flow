...
export class OpaquePakeService {
  ...
  async registrationStart(
    ...
    try {
      ...
      const response = await this.evaluateRegistration(
        request.clientRegistrationState,
        serverState
      );
  ...
  async registrationFinish(
    ...
    try {
      ...
      const userId = await this.storeOprfRecord(
        request.username,
        request.registrationRecord,
        request.serverRegistrationState
      );
  ...
  async loginStart(
    ...
    try {
      ...
      const serverState = this.generateServerState();
      const response = await this.evaluateLogin(
        request.clientLoginState,
        oprfRecord,
        serverState
      );
  ...
  async loginFinish(
    ...
    try {
      ...
      const sessionKey = await this.verifyLoginAndGenerateSession(
        request.clientLoginFinish,
        request.serverLoginState,
        request.username
      );
   ...
   */
  private generateServerState(): string {
   ...
   */
  private async getOprfRecord(username: string): Promise<string | null> {
    const db = this.fastify.db;
    const { oprfRecords, users } = await import('../lib/schema.js');
    ...
    const result = await db
      ...
      .where(
        and(
          ...
          gt(oprfRecords.expiresAt, new Date())
        )
      )
      .limit(1);
   ...
   * TODO: Replace with proper OPAQUE implementation
   */
  private async verifyLoginAndGenerateSession(
    _clientFinish: string, 
    _serverState: string, 
    _username: string
  ): Promise<string> {
   ...
   */
  private async getUserInfo(username: string): Promise<{ userId: string; tenantId: string }> {
    const db = this.fastify.db;
...
export const createOpaquePakeService = (fastify: FastifyInstance) => 
  new OpaquePakeService(fastify);
