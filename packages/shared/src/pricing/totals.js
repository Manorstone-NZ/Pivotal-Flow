 ...
 */
export function calculateQuoteTotals(lineCalculations, quoteDiscount) {
    ...
    let taxableAmount;
    if (quoteDiscount) {
        const discountCalculation = calculateDiscount(lineTotal, quoteDiscount.type, quoteDiscount.value);
 ...
 */
export function calculateQuoteTotalsWithBreakdown(lineCalculations, quoteDiscount) {
    // Calculate basic totals
    const totals = calculateQuoteTotals(lineCalculations, quoteDiscount);
    // Calculate tax breakdown from line items
    const taxBreakdown = calculateTaxBreakdown(lineCalculations.map(calc => ({
 ...
 */
export function calculateQuoteTotalsWithMultipleDiscounts(lineCalculations, quoteDiscounts) {
    const currency = lineCalculations[0]?.totalAmount.currency;
    if (!currency) {
        throw new Error('Cannot calculate totals for empty line calculations');
    }
    ...
    let discountAmount = { amount: new Decimal(0), currency };
 ...
 */
export function getTotalsBreakdown(totals) {
    return {
 ...
 */
export function validateQuoteTotals(totals) {
    try {
 ...
 */
export function calculateTotalsPercentages(totals) {
