 ...
 */
export function calculateLineItem(lineItem, currencyDecimals = 2) {
    // Validate inputs
    ...
    let extractedTaxAmount = { amount: new Decimal(0), currency: lineItem.unitPrice.currency };
    ...
    let discountAmount = { amount: new Decimal(0), currency: subtotal.currency };
    let taxableAmount = subtotal;
    // Apply percentage discount first (if specified)
    if (lineItem.percentageDiscount !== undefined && lineItem.percentageDiscount > 0) {
        const percentageDiscountCalculation = calculateDiscount(taxableAmount, 'percentage', lineItem.percentageDiscount);
    ...
    if (lineItem.fixedDiscount && lineItem.fixedDiscount.amount.greaterThan(0)) {
        const fixedDiscountCalculation = calculateDiscount(taxableAmount, 'fixed_amount', lineItem.fixedDiscount.amount.toNumber());
        discountAmount = {
    ...
    if (lineItem.discountType && lineItem.discountValue !== undefined) {
        const legacyDiscountCalculation = calculateDiscount(taxableAmount, lineItem.discountType, lineItem.discountValue);
        discountAmount = {
 ...
 */
export function calculateLineItems(lineItems, currencyDecimals = 2) {
    ...
    const subtotal = {
        amount: roundToCurrencyDecimals(calculations.reduce((sum, calc) => sum.plus(calc.subtotal.amount), new Decimal(0)), currencyDecimals),
        currency
    };
    const totalDiscount = {
        amount: roundToCurrencyDecimals(calculations.reduce((sum, calc) => sum.plus(calc.discountAmount.amount), new Decimal(0)), currencyDecimals),
        currency
    };
    const totalTaxable = {
        amount: roundToCurrencyDecimals(calculations.reduce((sum, calc) => sum.plus(calc.taxableAmount.amount), new Decimal(0)), currencyDecimals),
        currency
    };
    const totalTax = {
        amount: roundToCurrencyDecimals(calculations.reduce((sum, calc) => sum.plus(calc.taxAmount.amount), new Decimal(0)), currencyDecimals),
        currency
    };
    const totalAmount = {
        amount: roundToCurrencyDecimals(calculations.reduce((sum, calc) => sum.plus(calc.totalAmount.amount), new Decimal(0)), currencyDecimals),
        currency
    };
    return {
        calculations,
...
export function validateLineItem(lineItem) {
    try {
        ...
        if (lineItem.discountType && lineItem.discountValue !== undefined) {
            ...
            const discountCalculation = calculateDiscount(subtotal, lineItem.discountType, lineItem.discountValue);
 ...
 */
export function getLineItemBreakdown(calculation) {
    return {
