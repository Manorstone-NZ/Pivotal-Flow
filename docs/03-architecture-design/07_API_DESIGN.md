...
3. **Authentication & Authorization**: Opaque access tokens and PASETO refresh tokens with RBAC
...
7. **Validation**: Request/response validation using TypeBox schemas
...
#### **Opaque Access Token**
```typescript
interface IOpaqueAccessToken {
  access_token: string;  // Opaque token (base64url random bytes)
  token_type: 'Bearer';
...
#### **PASETO Refresh Token Structure**
```typescript
interface IPasetoRefreshToken {
  refresh_token: string; // PASETO v4.local encrypted token
  token_type: 'Bearer';
  expires_in: number;
  scope: string[];
}
...
// Example Response
{
  "access_token": "v2.local.BEsKs2AO2SK...",
  "refresh_token": "v4.local.encrypted_refresh_payload...",
  "token_type": "Bearer",
  "expires_in": 900,
  "user": {
    "id": "user_123",
...

{
  ...
  "end_time": "17:00",
  "duration_hours": 8.0,
  "description": "Implemented user authentication system with Opaque + PASETO tokens",
  "billable": true,
  "hourly_rate": 100.00,
  "tags": ["authentication", "paseto", "backend"]
}
  ...
  schemas:
    ...
    LoginResponse:
      ...
      properties:
        ...
        refresh_token:
          type: string
          description: PASETO refresh token
        token_type:
          type: string
          enum: [Bearer]
          default: Bearer
