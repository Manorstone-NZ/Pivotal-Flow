...
export class TokenAuthGuard implements CanActivate {
  constructor(
    private readonly tokenService: PasetoTokenService,
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    try {
      const sessionData = await this.tokenService.validateAccessToken(token);
      const user = await this.userService.getUserById(sessionData.userId);
