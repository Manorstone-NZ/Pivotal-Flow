...
export const requireAuth = (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    const sessionData = await tokenService.validateAccessToken(token);
...
export const tokenSecurityTests = {
  opaqueTokens: {
    description: 'Validate opaque access token security',
    tests: [
      'Token generation produces random opaque strings',
      'Token validation requires Redis session data',
      'Expired tokens are properly rejected',
      'Revoked tokens cannot be reused',
      'Token binding validates IP and User-Agent'
    ]
  },
  pasetoTokens: {
    description: 'Validate PASETO refresh token security',
    tests: [
      'PASETO tokens use v4.local encryption',
      'Token payload is properly encrypted',
...
export const tenantSecurityTests = {
  ...
  permissionIsolation: {
    description: 'Validate tenant permission isolation',
    tests: [
      'Permissions are scoped to tenant context',
      'Role assignments are tenant-specific',
      'Resource access respects tenant boundaries',
...
export const cryptographicTests = {
  passwordHashing: {
    description: 'Validate password hashing security',
    tests: [
  ...
  keyManagement: {
    description: 'Validate cryptographic key management',
    tests: [
      'PASETO keys are generated securely',
      'Key rotation works without downtime',
      'Keys are stored securely',
      'Key versioning supports multiple keys',
      'Key derivation uses secure algorithms'
    ]
  }
};
```
