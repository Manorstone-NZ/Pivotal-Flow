...
interface IAuthResponse {
  ...
  refresh_token: string; // PASETO refresh token
  token_type: 'Bearer';
...
interface IOpaqueAccessToken {
  token: string;           // Base64url encoded random bytes (32 bytes)
  expires_in: number;      // 900 seconds (15 minutes)
...
### **PASETO Refresh Tokens**
```typescript
interface IPasetoRefreshToken {
  token: string;           // PASETO v4.local encrypted payload
...
- **Quantum-Resistant**: Uses XChaCha20-Poly1305 encryption
...
- [ ] Opaque access token and PASETO refresh token generation and validation
